...
function openQC(b,p,s,q){
    ...
    mQty.innerText=q;
    receivedQtyHidden.value = q;
    qcModal.classList.remove("hidden");
}
function closeQC(){ qcModal.classList.add("hidden"); }

function validateQuantities(){
    const received = parseInt(receivedQtyHidden.value || 0);
    const damaged = parseInt(document.querySelector('input[name="quantityDamaged"]')?.value || 0);
    const returned = parseInt(document.querySelector('input[name="quantityReturn"]')?.value || 0);
...
function showGood(){ goodBox.classList.remove("hidden"); badBox.classList.add("hidden"); updateSaveState(); }
function showBad(){ badBox.classList.remove("hidden"); goodBox.classList.add("hidden"); updateSaveState(); }
function showReturn(){ returnBox.classList.remove("hidden"); updateSaveState(); }
function hideReturn(){ returnBox.classList.add("hidden"); updateSaveState(); }

function prepareRemarks(){
    finalRemarks.value =
        !goodBox.classList.contains("hidden")
        ? goodRemarks.value
        : badRemarks.value;
}

function updateSaveState(){
    const good = document.querySelector('input[value="GOOD"]')?.checked;
    const bad = document.querySelector('input[value="DAMAGED"]')?.checked;

    if (good) return saveBtn.disabled = false;
    if (!bad) return saveBtn.disabled = true;

    const damaged = document.querySelector('input[name="quantityDamaged"]')?.value;
    if (!damaged || damaged <= 0) return saveBtn.disabled = true;

    if (document.querySelector('input[value="Y"]')?.checked) {
        const returned = document.querySelector('input[name="quantityReturn"]')?.value;
        if (!returned || returned <= 0) return saveBtn.disabled = true;
    }
    saveBtn.disabled = false;
}

/* ===============================
   AUTO FILTER + PAGINATION (MATCH PRODUCT JSP)
   =============================== */
...
function paginateTable() {
    ...
    // MATCH PRODUCT JSP BEHAVIOR
    if (matchedRows.length <= rowsPerPage) {
        pagination.style.display = "none";
        document.getElementById("tableFooter").style.justifyContent = "flex-end";
        matchedRows.forEach(row => row.style.display = "");
        return;
    }

    pagination.style.display = "flex";
    document.getElementById("tableFooter").style.justifyContent = "space-between";
    ...
    if (start > 1) { addPage(1); addDots(); }
    for (let i = start; i <= end; i++) addPage(i);
    if (end < totalPages) { addDots(); addPage(totalPages); }
    ...
    function addPage(page) {
        ...
        btn.className =
            page === currentPage
                ? "px-3 py-1 rounded-full bg-indigo-100 font-bold cursor-default"
    ...
    function addNav(label, disabled, action) {
        ...
        btn.className = disabled
            ...
            : "px-3 py-1 hover:bg-white cursor-pointer";
