...
List<Map<String, String>> qcList = (List<Map<String, String>>) request.getAttribute("qcList");
...
<body class="bg-[#fdf8e9] h-screen flex flex-col">
	...
	<div class="flex flex-1 overflow-hidden">
		...
		<main class="flex-1 p-8 overflow-y-auto">
			...
			<div
				...
				<h2 class="text-3xl font-black text-cyan-900 mb-4">PRODUCT
					QUALITY CHECK</h2>
				...
				<div class="flex justify-between items-center mb-6">
					...
					<div class="flex gap-3">
						<a href="?tab=pending"
							...
							Pending QC </a> <a href="?tab=completed"
            ...
            '<%=r.get("qty")%>'
							...
							<td><a
								href="<%=request.getContextPath()%>/product-qc?action=view&qcId=<%=r.get("qcId")%>"
								class="view-icon" title="View QC"> <i class="fas fa-eye"></i>
							</a></td>

							<td>
								<%
								if ("DAMAGED".equals(r.get("condition")) && r.get("returnQty") != null && Integer.parseInt(r.get("returnQty")) > 0) {
								%> <a
								href="<%=request.getContextPath()%>/return-note?qcId=<%=r.get("qcId")%>"
								class="btn-return"> <i class="fas fa-print"></i> Return Note
							</a> <%
 } else {
 ...
 %>
						...
						%>
						<tr>
							<td colspan="8" class="py-6 text-center text-gray-500">No
								records found</td>
						</tr>
						<%
						}
	...
	<form method="post" action="<%=request.getContextPath()%>/product-qc">
		<div id="qcModal"
			...
			<div
				class="bg-white w-[900px] max-h-[90vh]
 ...
 translate-x-6 -translate-y-6">
				...
				<div
					class="bg-green-50 border border-green-200 rounded-2xl p-6 mb-8
            grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4 text-sm">
					...
					<p class="flex gap-2">
						<span class="font-bold text-gray-800 w-36 uppercase tracking-wide">Checked
							By</span> <span class="text-gray-900">: <%=staffName%></span>
					</p>

					<p class="flex gap-2">
						<span class="font-bold text-gray-800 w-36 uppercase tracking-wide">Quantity
							Received</span> <span class="text-gray-900">: <span id="mQty"></span></span>
					</p>

					<p class="flex gap-2">
						<span class="font-bold text-gray-800 w-36 uppercase tracking-wide">Quality
							Check Date</span> <span class="text-gray-900">: <%=new java.sql.Date(System.currentTimeMillis())%></span>
					</p>
				...
				<input type="hidden" name="remarks" id="finalRemarks"> <input
					type="hidden" id="receivedQtyHidden" name="receivedQty">

				<!-- CONDITION -->
				<div class="mb-4">
					<label class="font-bold">Inspection Result</label>
					<div class="flex gap-8 mt-2">
						<label> <input type="radio" name="condition" value="GOOD"
							onclick="showGood()" required> Good
						</label> <label> <input type="radio" name="condition"
							value="DAMAGED" onclick="showBad()"> Damaged / Expired
						</label>
					</div>
				</div>
				...
				<!-- GOOD -->
				<div id="goodBox"
					class="hidden border rounded-xl p-4 mb-4 bg-gray-50">
					<label class="font-bold">Remarks</label>
					<textarea id="goodRemarks" class="w-full border rounded p-2"
				...
				<div id="badBox" class="hidden border rounded-xl p-4 mb-4 bg-red-50">
					...
					<label class="font-bold">Quantity Damaged</label> <input
						name="quantityDamaged" type="number" min="1"
						class="w-full border rounded p-2 mb-3"> <label
						class="font-bold">Returnable</label>
					<div class="flex gap-6 mt-2 mb-3">
						<label class="flex items-center gap-2"> <input
							type="radio" name="returnable" value="Y" onclick="showReturn()">
							Yes
						</label> <label class="flex items-center gap-2"> <input
							type="radio" name="returnable" value="N" onclick="hideReturn()">
							No
						</label>
					</div>

					<div id="returnBox" class="hidden mb-3">
						<label class="font-bold">Quantity to Return</label> <input
							name="quantityReturn" type="number" min="1"
							class="w-full border rounded p-2">
					</div>

					<label class="font-bold">Remarks</label>
					<textarea id="badRemarks" class="w-full border rounded p-2"
				...
				<div class="flex justify-end gap-4">
					...
					<button type="button" onclick="closeQC()"
	...
	function paginateTable() {
	    ...
	    const rows = Array.from(document.querySelectorAll("#qcTable tr"));
	    ...
	    prev.className = "px-3 py-1 rounded-full " +
	    (prev.disabled
	    ...
	    next.className = "px-3 py-1 rounded-full " +
	    (next.disabled
	        ? "text-gray-400 cursor-not-allowed"
	        : "hover:bg-white cursor-pointer");
	    next.onclick = () => {
	    ...
	    function addPage(p) {
	        ...
	        btn.className =
	            p === currentPage
	                ? "px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 font-bold cursor-default"
	                : "px-3 py-1 rounded-full hover:bg-white cursor-pointer";
